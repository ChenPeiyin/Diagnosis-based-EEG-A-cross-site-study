...
def inter_set_xgb(ratio):
    ...
    xgboost_model = xgboost.XGBClassifier(max_depth=4, eval_metric=['logloss', 'auc', 'error'])
    xgboost_model.fit(train_x, train_y.reshape(-1, ))

    predicted_y_prob = xgboost_model.predict_proba(test_x)[:, 1]
...
def intra_set_xgb():
    ...
    for trn_idex, tst_idex in k_fold.split(data, label):
        ...
        xgboost_model = xgboost.XGBClassifier(eval_metric=['logloss', 'auc', 'error'])
        xgboost_model.fit(train_x, train_y.reshape(-1, ))
...
if __name__ == '__main__':
    ...
    all_auc = []
    for i in range(100):
        acc, recall, precision, f1, specificity, auc = inter_set_xgb(ratio)
        # acc, recall, precision, f1, specificity, auc = intra_set_xgb()
        all_acc.append(acc)
        all_recall.append(recall)
        all_precision.append(precision)
        all_f1.append(f1)

...
def inter_set_RF(ratio):
    ...
    predicted_y_prob = RF.predict_proba(test_x)[:, 1]
...
if __name__ == '__main__':
    ...
    all_auc = []
    for i in range(500):
        acc, recall, precision, f1, specificity, auc = inter_set_RF(ratio)
        # acc, recall, precision, f1, specificity, auc = intra_set_RF()
        all_acc.append(acc)
        all_recall.append(recall)
        all_precision.append(precision)
        all_f1.append(f1)
